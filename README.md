...
```ini
# -----------------------------------------------------------------------------
# Токен телеграм-бота, секретный ключ Джанго сервера
# -----------------------------------------------------------------------------
...
# Уведомления старше 15 минут
NOTIFICATION_CLEANUP_MINUTES=15

# Задача очистки запускается каждые X часов
NOTIFICATION_CLEANUP_INTERVAL_HOURS=8
```

Ещё один вариант, в качестве улучшения структуризации - создать папку `.env_collection` в корне проекта (также перед запуском) и заполнить её `.env` файлами, которые будут содержать
...
А затем произвести их мерж командой из корневой директории: `python scripts/env_merge.py`. 
